...
def cal_ccs(mz_measured, drift_time, beta, tfix):
    import math
    mi = mz_measured
    tA = drift_time
    m = np.add(28.04, mi)
...
class MainApp(QMainWindow , ui):
    def __init__(self):
        ...
        self.svg.currentTextChanged.connect(self.combobox_svg)
        ...
        self.primary_data = {"primary_ion": "", "drift_gas": "", "mzml": "", "beta": "", "tfix": "", "buffer_text": "", "chargestate": '', "ion_intensity": '', 'use_data': '', 'svg': 5}
    ...
    def download3(self):
        ...
        def done(result):
            if not bool(self.n):
                ...
                f = open(path, "r")
                QMessageBox.information(self, "Information", f"The output results were obtained successfully and {f.read()}")
                f.close()
    ...
    def combobox_svg(self, s):
        if s == "noise level used in svg":
            s = 5.0
        self.primary_data["svg"] = float(s)



    def noise_combobox_text(self, s):
        if s == "PPM Tolerance":
            s = 0.0
        self.secondary_data["mono_combobox"] = s
    ...
    async def find_ccs(self):
            ...
            self.ccs_table["PubChem_CID"] = " "
            self.ccs_table["drift_time"] = self.ccs_table["drift_time"].map(lambda x: float(x)).map(lambda x: round(x, 3))
            self.ccs_table["rt"] = self.ccs_table["rt"].map(lambda x: float(x)).map(lambda x: round(x, 3))
            if "Alert" in self.message.keys():
                self.message["warning"].append(f"The output not obtained, because of {str(self.message['Alert'])}")
                return None
            self.optional_data = defaultdict(lambda: None)
            show_table = self.ccs_table[['PubChem_CID', 'Compound_name', 'molecular_formula',  'ion_type',  'mz_measured', "Error(PPM)", "#conformer", "drift_time", "ccs",  "resolving_power", "rt"]]
            show_table.columns = ["PubChem_CID", "Compound_name", "Molecular_formula", "Ion_type", "mz_measured", "Error_ppm", "Conformer", "Arrival_time_ms", "CCS_sqA", "Resolving_power", "RT_min"]
    ...
    def plot_all_rt(self):
        try:
            ...
            if len(xx) > 1:
                ll = list(xx.values())
                df = pd.concat(ll)
                df = df[df['ion_type'] == self.primary_data["primary_ion"]]
                x = df['rt'].values
    ...
    def plot_all_dt(self):
        try:
            ...
            if len(xx) > 1:
                ...
                df = pd.concat(ll)
                df = df[df['ion_type'] == self.primary_data["primary_ion"]]
                df = df[["drift_time", "intensity", "mf_rt"]]
    ...
    def download_result(self):
        ...
        try:
            ...
            make_all = []
            whole = namedtuple('Whole', ["PubChem_CID", "Compound_name", "Molecular_formula", "Ion_type", "mz_measured", "Error_ppm", "Conformer", "Arrival_time_ms", "CCS_sqA", "Resolving_power", "RT_min"])
            ...
            df['Arrival_time_ms'] = df['Arrival_time_ms'].map(lambda x: float(x))
            df['CCS_sqA'] = df['CCS_sqA'].map(lambda x: float(x))
            df['Resolving_power'] = df['Resolving_power'].map(lambda x: float(x))
            df["Formula_RT"] = df["Molecular_formula"] + df["RT_min"].astype(str)
            df['RT_min'] = df['RT_min'].map(lambda x: float(x))
            df["PubChem_CID"] = df["PubChem_CID"].map(lambda x: f'https://pubchem.ncbi.nlm.nih.gov/compound/{x}')
            import datetime
            now = datetime.datetime.now()
            df["Time"] = now
            for index, aa in df.iterrows():
                name_im = aa["Molecular_formula"] + "_" + aa["Ion_type"] + "_" + str(aa["RT_min"]) + "_" + "IM" + ".html"
                myfile = os.path.join(output, name_im)
                shutil.copy2(myfile, path_text)
                name_im1 = aa["Molecular_formula"] + "_" + str(aa["RT_min"]) + "_" + "IMoverlay" + ".html"
                myfile1 = os.path.join(output, name_im1)
                shutil.copy2(myfile1, path_text)
                name_iso = aa["Molecular_formula"] + "_" + aa["Ion_type"] + "_" + str(aa["RT_min"]) + ".html"
                myfile2 = os.path.join(output1, name_iso)
                shutil.copy2(myfile2, path_text)
                name_iso = aa["Molecular_formula"] + "_" + "rt" + "_" + "overlay" + ".html"
                myfile3 = os.path.join(output1, name_iso)
                shutil.copy2(myfile3, path_text)
        except Exception as e:
            print(e)
            pass
    ...
    def commit_database(self):
        try:
            ...
            if button == QMessageBox.StandardButton.Yes:
                ...
                df2.columns = ["Molecular_formula", "RT_min", "Ion_type", "Intensity"]
                df2["Time"] = now
                df11 = df[["Molecular_formula", "Ion_type"]]
                df2 = df2.merge(df11, how='inner', on= ["Molecular_formula", "Ion_type"])
                ll = []
                xx = self.launcher.all_dt
                for x in xx.keys():
                    mm = xx[x]
                ...
                df3.columns = ["Molecular_formula", "RT_min", "Ion_type", "Intensity", "Arrival_time_ms", "Formula_RT"]
                df11 = df[["Molecular_formula", "Ion_type", "RT_min"]]
                df3 = df3.merge(df11, how='inner', on= ["Molecular_formula", "Ion_type", "RT_min"])
                df3["Time"] = now
                ...
                df4 = pd.DataFrame({"Drift_gas": zz, "beta_slope": kk, "tfix_intercept": gg, "Field_strength_V_cm": x1, "Drift_tube_pressure_Pa": x2, "Drift_tube_temperature_Kelvin": x3, "LC_method_number": x4, "processing_applied": x5, "Time": now}, index = [0])
                df2.to_sql(name='EICs', con=conn, if_exists='append', index=False)
                df3["beta_slope"] = kk
                df3["tfix_intercept"] = gg
                df11 = df[["Molecular_formula", "Ion_type", "mz_measured"]]
                df3 = df3.merge(df11, how='left', on= ["Molecular_formula", "Ion_type"])
                df3["CCS"] = df3.apply(lambda x: cal_ccs(x['mz_measured'], x['Arrival_time_ms'], x['beta_slope'], x['tfix_intercept']), axis=1)
                df3.dropna(inplace = True)
                df3 = df3[df3["CCS"] > 0]
                df3.to_sql(name='IM_spectra', con=conn, if_exists='append', index=False)
