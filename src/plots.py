...
def filter(xx, svg):
    xx = np.array(xx)
    return savgol_filter(xx, svg, 2, mode='nearest')
...
class Xic_Eic:
    ...
    def __init__(self, data):
        ...
        self.svg = self.data.svg


    def plotrt_EIC(self, molecular_formula):
        ...
        try:
            ...
            for ion in ff:
                ...
                df22["intensity"] = df22["intensity"].fillna(method='ffill')
                df22 = df22.sort_values(by = ["intensity"], ascending = False)
                df22 = df22.drop_duplicates(subset = ["rt"])
                df22["intensity"]= filter(df22["intensity"].values, self.svg)
                df_all.append(df22)
            df = pd.concat(df_all)
            ...
            for rt in rt_list:
                ...
                if label in list(self.data.rt_found_ion.keys()):
                    ...
                    for ion in ion_list:
                        ...
                        y = savgol_filter(y, self.svg, 2, mode='mirror')
                        ...
                        ynew = list(ynew)
                        ynew = [max(num, 0) for num in ynew]
                        ynew = savgol_filter(ynew, self.svg, 2, mode='mirror')
                        ynew = [max(num, 0) for num in ynew]
                    ...
                    y= df22['intensity'].values
                    b = str(rt)
                    fig = px.line(df22, x="drift_time", y="intensity", color='ion_type', title=f'overlay_IM plot of {label}')
                    fig.write_html(os.path.join(self.data.temp_drift, f"{self.data.molecular_formula}_{b}_IMoverlay.html"))
                    df_all1[f"{b}"] = df22

            kk = []
            pp = list(df_all1.values())
            df33 = pd.concat(pp)
            print(df33.head())
            self.data.all_dt[self.data.molecular_formula] = df33
        except Exception as e:
            return e
