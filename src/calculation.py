...
class Calculation:
    ...
    def rt_window_formula(self):
        ...
        if index_size > 5:
            pp, peak = detect_rt(mm, self.noise, self.svg)
            if peak:
            ...
            for i in range(len(pp)):
                ...
                if final_rt.size > 0:
                    ...
                    if not isinstance(intensity, type(None)):
                        ...
                        start, peaks, end = detect_peaks(intensity, noise, baseline, self.noise, self.svg)
                        ...
                        df = self.peak_dt(lt, ion, rt, mz, rtE, self.svg)
                        if self.peaks:
                            label = self.molecular_formula + str(rtE)
    ...
    def peak_dt(self, df, z, i, mz1, rtE, svg):
