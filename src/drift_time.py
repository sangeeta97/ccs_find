...
class Parse_MF:
    ...
    temp_spectrum, temp_isotope stores the result files and viewed upon calling the tool
    ...
    def __init__(self, data):
        ...
        self.svg = self.data.primary_data["svg"]
    ...
    def mix_all(self, x, y):
        ...
        if self.changing['status']:
            rt_list = self.found_molecular_formula[x]
            if rt_list.size > 0:
                calc_object.ion_dataframe()
                plot_object = Xic_Eic(self)
                plot_object.plotrt_EIC(x)
                plot_object.plotdt_IC(x)
                for rt in rt_list:
